...
export function FilesTable({ files, handleDeleteFile, processingProgress }: FilesTableProps) {
  return (
        ...
        {files.length > 0 ? (
          <TableBody>
            {files.map(({ file, preview: previewURL, duration, title }, index) => {
              const progress = processingProgress[file.name] ?? 0
              const seconds = Math.floor(Number(duration))
              const secondDuration = dayjs().startOf('day').second(seconds).format(seconds >= 3600 ? 'HH:mm:ss' : 'mm:ss')
                  ...
                  <TableCell>
                    <div className='flex flex-col'>
                      ...
                      <span className="text-xs text-zinc-500">
                        {(file.size / (1024 * 1024)).toFixed(2)} MB • {secondDuration}
                      </span>
                    </div>
                  </TableCell>
                  <TableCell>
