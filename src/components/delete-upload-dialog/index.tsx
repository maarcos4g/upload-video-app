...
import { useDeleteUpload } from "@/http/delete-upload";
import { handleDeleteUpload } from "./actions";

type DeleteUploadDialogProps = {
  upload: GetUploadResponse['uploads'][0]
}

export function DeleteUploadDialog({ upload }: DeleteUploadDialogProps) {

  const { slug } = useCurrentOrganization()

  const [verify, setVerify] = useState('')

  const isDisabled = verify.toLowerCase().trim() !== 'deletar upload'.trim()

  const { mutateAsync: deleteUpload } = useDeleteUpload()

  const [_, handleSubmit, isPending] = useFormState(
    (data) => handleDeleteUpload(data, deleteUpload)
  )
    ...
    >
      <DialogHeader>
        <div className="bg-zinc-950 px-4 py-6 rounded-t-md border-b border-zinc-700 space-y-4">
          <DialogTitle>Deletar Upload</DialogTitle>
          <DialogDescription className="text-sm text-zinc-500">
            Tem certeza que deseja excluir esse upload?
          </DialogDescription>
        </div>
      </DialogHeader>
      ...
      <form className="flex flex-col px-4 pb-6 gap-4" onSubmit={handleSubmit}>
        <div className="flex flex-col gap-1">
          ...
          >
            Para confirmar, digite <strong>deletar upload</strong> abaixo:
          </label>
        ...
        >
          ...
          A ação de deletar este título não poderá ser revertida.
        </div>
